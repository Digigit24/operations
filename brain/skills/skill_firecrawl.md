...
id: "skill_firecrawl"
type: "skill"
when_to_use: "When you need to scrape, crawl, or extract content from websites — competitor sites, client sites, research sources"
related:
  - skill_onboarding_03_site_extraction
...
- **Human pastes it in** — set `FIRECRAWL_API_KEY=fc-...` in your environment or pass it directly
...
## Usage in the Prompt Chain

Firecrawl is the preferred tool for **Onboarding Step 03 — Site Extraction** (`onboarding/03-site-extraction.md`).

When running Step 03:
- Use `firecrawl_crawl` to crawl the full client website (up to 50 pages)
- Use `firecrawl_scrape` for individual page extraction if needed
- Tavily MCP is the fallback if Firecrawl is not connected

Firecrawl is also useful in **M1 Audit §5.5** when re-checking live page content against what was extracted at onboarding.
